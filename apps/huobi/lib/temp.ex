defmodule WebSocketExample do
  ...
  def handle_message_ticker(%{"tick" => tick, "ch" => ch} = msg, state) do
    ...
    # check if at least we received an orderbook once
    if Map.has_key?(new_state[pair].orderbook, :bid) do
      IO.puts("Got here")
      # AggregatorActor.new_message(new_state[pair])
    else
      IO.puts("have not received an orderbook")
    end
